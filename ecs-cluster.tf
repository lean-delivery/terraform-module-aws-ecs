...
locals {
  ecs_cluster_id = element(concat(aws_ecs_cluster.this.*.id, [var.ecs_cluster_id]), 0)
  ecs_cluster_arn = element(
    concat(aws_ecs_cluster.this.*.arn, data.aws_ecs_cluster.this.*.arn),
    0,
  )
  ecs_cluster_name = element(
    concat(
