output "ecs_service_iam_role_arn" {
  value = element(concat(aws_iam_role.ecs-service.*.arn, aws_iam_role.ecs-service-ec2.*.arn, tolist([""])), 0)
  description = "ARN fo created ECS service"
}

output "ecs_service_iam_role_name" {
  value = element(concat(aws_iam_role.ecs-service.*.name, aws_iam_role.ecs-service-ec2.*.name, tolist([""])), 0)
